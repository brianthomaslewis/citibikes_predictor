...
def process_bike_data(trips_df, stations_df, rebalancing_prop):
    ...
    Args:
        trips_df (pandas DataFrame): DataFrame consisting of the trips data acquired
            from Citi Bike
        stations_df (pandas DataFrame): DataFrame consisting of the stations
            data acquired from Citi Bike
        rebalancing_prop (double): Proportion from 0-1 to indicate how much
            daily rebalancing occurs at stations
    Returns:
    ...
    try:
        ...
        bikes['name'] = bikes.groupby('station_id')['name'].\
            transform(lambda x: x.ffill().bfill())
        bikes['latitude'] = bikes.groupby('station_id')['latitude'].\
            transform(lambda x: x.ffill().bfill())
        bikes['longitude'] = bikes.groupby('station_id')['longitude'].\
            transform(lambda x: x.ffill().bfill())
        bikes['capacity'] = bikes.groupby('station_id')['capacity'].\
            transform(lambda x: x.ffill().bfill())
        bikes['net_flows'] = bikes['outflows'] - bikes['inflows']

        # Calculate stock of bikes, using `rebalancing_prop`
        bikes['stock'] = np.where(
            bikes.groupby(['station_id', 'date'])['hour'].
            transform('min') == bikes['hour'],
        ...
        bikes_df = bikes.drop(['capacity', 'inflows', 'hour', 'outflows',
                               'net_flows'], axis=1).dropna()

    except AttributeError:
        logger.error(
            "Your input to the function 'process_bike_data' was not a "
            "DataFrame and thus the function could not run")
        sys.exit(1)
    except ValueError as err_msg:
        logger.error("Unexpected ValueError in the function 'process_bike_data':"
                     " %s:%s", type(err_msg).__name__, err_msg)
        sys.exit(1)
...
def run_data_processing(arguments):
    """
    Wrapper function that processes and runs ARIMA on the raw data to
    obtain results for database upload
    Args:
        arguments: From argparse:
           config (str): Path to yaml file with containing relevant configurations
           engine_string (str): sqlalchemy engine string
           local_flag (bool): the flag used to determine if the scripts
            will read/write via s3 or local
           s3_bucket (str): name of the S3 bucket to pull data from
    Returns:
        None -- this a wrapper function for the data preparation steps
    """
    try:
        with open(arguments.config, "r") as fil:
            config = yaml.load(fil, Loader=yaml.FullLoader)
    except IOError:
        logger.error("Could not read in the config file--verify correct "
                     "filename/path.")
        sys.exit(1)
    ...
    # Otherwise, open up from S3
    else:
        logger.info('Retrieving data from S3 at %s', arguments.s3_bucket)
        trips = download_csv_s3(s3_bucket_name=arguments.s3_bucket,
                                bucket_dir_path=
                                config['download_csv_s3']['trips_data']['bucket_dir_path'],
                                input_filename=
                                config['download_csv_s3']['trips_data']['input_filename'],
                                output_filename=
                                config['download_csv_s3']['trips_data']['output_filename'])
        stations = download_csv_s3(s3_bucket_name=arguments.s3_bucket,
                                   bucket_dir_path=
                                   config['download_csv_s3']['stations_data']['bucket_dir_path'],
                                   input_filename=
                                   config['download_csv_s3']['stations_data']['input_filename'],
                                   output_filename=
                                   config['download_csv_s3']['stations_data']['output_filename'])

    # Begin processing bike data
    logger.info('Processing trips and stations data for modeling...')
    bike_df = process_bike_data(trips, stations,
                                config['process_bike_data']['rebalancing_proportion'])

    # Save bike_stock data locally
    bike_df.to_csv(config['process_bike_data']['output_file'], index=False)
    logger.info('Success! Added bike_stock data locally to:'
                ' "%s"', config['process_bike_data']['output_file'])
    ...
    try:
        logger.info('Attempting to add bike stock data to '
                    'database at %s...', arguments.engine_string)
        add_to_database(bike_df, "bike_stock", 'replace', arguments.engine_string)
        logger.info('Success! Added data to "bike_stock" '
                    'table at the following engine_string: %s',
                    arguments.engine_string)
        logger.info("data_processing.py was run successfully.")

    except ValueError as err_msg:
        logger.error('Bikes data unable to be added to "bike_stock"'
                     ' database. Try checking the dimensions of the data'
                     'and try again.')
